...
- “已撤回 B/S”开关显示某个历史截止日曾经可见、但在当前截止日已经撤回的候选；空间足够时显示灰色 `B---->x/S---->x`，起点为原候选日，末端 `x` 为首次消失日。缩小图表或没有足够避让空间时，在原信号出现日显示紧凑的 `Bx/Sx`，并沿用普通 B/S 的标记堆叠，避开连续数字。原信号日期不在所选范围内时仅在下方撤回记录表中保留记录。图下方的撤回记录表列出两端的确切日期；它们不混入正式信号。
